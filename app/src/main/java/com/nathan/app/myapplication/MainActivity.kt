...
class MainActivity : AppCompatActivity() {
    ...
    val stratDescription = arrayListOf(
        //one mic
...
just armor. Your team must surround the president and protect him/her for the entire round."""
    ...
    fun stratDisplayRoll(title: TextView, description: TextView){
        var numberSet = 0
        while (numberSet < 6) {
            var randomNumber = (0..29).random()
            title.text = stratTitle[randomNumber]
            description.text = stratDescription[randomNumber]
            numberSet++
        }
    }
